...
implementation
    ...
    uses
       ...
       globtype,globals,systems,verbose,comphook,cscript,fmodule,i_optim8os,link,
    ...
    type
       ...
       TLinkerOptim8OS=class(texternallinker)
       private
          FOrigin: Word;
       ...
       TInternalLinkerOptim8OS=class(tinternallinker)
       private
         FOrigin: Word;
       protected
...
function TLinkerOptim8OS.WriteResponseFile_Sdld: Boolean;
  Var
    linkres  : TLinkRes;
    s        : TCmdStr;
    prtobj: string[80];
...
function TLinkerOptim8OS.WriteResponseFile_Vlink: Boolean;
  Var
    linkres  : TLinkRes;
    s        : TCmdStr;
    prtobj: string[80];
...
procedure TLinkerOptim8OS.SetDefaultInfo_Sdld;
  const
    ExeName='sdldz80';
  begin
...
procedure TLinkerOptim8OS.SetDefaultInfo_Vlink;
  const
    ExeName='vlink';
  begin
    if ImageBaseSetExplicity then
...
procedure TLinkerOptim8OS.SetDefaultInfo;
  begin
...
function TLinkerOptim8OS.MakeExecutable_Sdld: boolean;
  var
    binstr,
    cmdstr,
    mapstr: TCmdStr;
...
function TLinkerOptim8OS.MakeExecutable_Vlink: boolean;
  var
    binstr,
    cmdstr: TCmdStr;
    success: boolean;
...
function TLinkerOptim8OS.MakeExecutable: boolean;
  begin
    if not (cs_link_vlink in current_settings.globalswitches) then
...
procedure TLinkerOptim8OS.InitSysInitUnitName;
begin
  sysinitunit:='si_prc';
end;

function TLinkerOptim8OS.postprocessexecutable(const fn: string; isdll: boolean): boolean;
  begin
...
{*****************************************************************************
                          TInternalLinkerOptim8OS
*****************************************************************************}

procedure TInternalLinkerOptim8OS.DefaultLinkScript;
  var
    s        : TCmdStr;
    prtobj: string[80];
  begin
...
constructor TInternalLinkerOptim8OS.create;
  begin
    inherited create;
    CArObjectReader:=TArObjectReader;
    CExeOutput:=TOptim8OSIntelHexExeOutput;
...
procedure TInternalLinkerOptim8OS.InitSysInitUnitName;
  begin
    sysinitunit:='si_prc';
  end;

function TInternalLinkerOptim8OS.MakeExecutable: boolean;
  begin
    result:=inherited;
    { Post process }
...
function TInternalLinkerOptim8OS.postprocessexecutable(const fn: string): boolean;
  var
    exitcode: longint;
    FoundBin: ansistring;
    Found: Boolean;
...
initialization
{$ifdef z80}
  RegisterLinker(ld_int_optim8os,TInternalLinkerOptim8OS);
  RegisterLinker(ld_optim8os,TLinkerOptim8OS);
  RegisterTarget(system_z80_optim8os_info);
{$endif z80}
end.
