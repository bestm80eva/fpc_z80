               ...
               while j<=length(More) do
                begin
                  case More[j] of
                    ...
                    'B':
                      begin
                        if target_info.system in systems_all_windows+systems_symbian+[system_z80_zxspectrum, system_z80_optim8os] then
                          begin
...
{ Temporary defines, until things settle down }
  ...
  if target_info.system in (systems_embedded+systems_freertos+[system_z80_zxspectrum,system_z80_optim8os, ,system_z80_msxdos]) then
    begin
      case target_info.system of
{$ifdef AVR}
        system_avr_embedded:
...
{$ifdef cpufpemu}
  ...
  if not(option.FPUSetExplicitly) and
     ((target_info.system in [system_arm_wince,system_arm_gba,
         ...
         system_z80_embedded,system_z80_zxspectrum,system_z80_optim8os])
{$ifdef arm}
